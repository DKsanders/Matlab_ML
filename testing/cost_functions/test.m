...
cost_function{3} = MultiLabelClassificationCostFunction;
activation_function{3} = SoftmaxActivation;
y{3} = randi([0,9],num_cases,1);
...
for i=1:length(cost_function)
    ...
    for j = 1:num_features
        for k = 1:num_outputs
            ...
            cost_plus = cost_function{i}.cost(x, y{i}, y_plus);
            
            w_minus = w;
            w_minus(j,k) = w_plus(j,k) - epsilon;
            y_minus = activation_function{i}.activation(x * w_minus);
            cost_minus = cost_function{i}.cost(x, y{i}, y_minus);
            numeric_derivative(j,k) = (cost_plus - cost_minus) / (2*epsilon);
        end
    end
    
    if (sum(sum((real_derivative - numeric_derivative) > epsilon/10))  > 0)
        cost_function{i}
    end
end