...
%--------------------------------------------------------------------------
    ...
    methods (Hidden = true, Access = public)
        ...
        %
        % Custom function to make move based on piece, id and change
        %  Piece: 1:Pawn, 2:Knight, 3:Bishop, 4:Rook, 5:Queen 6:King
        %  ID: each piece's ID e.g. 1~8 for pawns, 1 for King
        %  Change: from - to on a linear board (-63~63)
        %
        function [SANstr] = MyMove(this, piece, id, change)
            SANstr = '';
            [is_valid,fromi,fromj,toi,toj,pID] = this.BS.GetMyMove(piece, id, change, this.turnColor);
            if (is_valid == true)
                [SANstr eog] = this.LegalMove(fromi,fromj,toi,toj,pID);
            end
        end
